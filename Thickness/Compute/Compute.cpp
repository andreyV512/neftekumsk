...
//-----------------------------------------------------------------------------------------
void Compute::EndAdjustmentsTube()//коррекци€ результата с учЄтом мЄртвой зоны в конце трубы
{
...
{
#ifdef DEBUG_ITEMS
	Communication::GetOffset();//получены временные смещени€
#endif
		...
		if(TryEnterCriticalSection(&cs1))
		{
			...
			for(;;)
			{
				...
				for(unsigned lz = start - offs; lz < stop - offs; ++lz)
				{
					...
					//if(deadAreaTime < lz)//изменено 18 11 14 если данные в мЄртвой зоне вычислени€ не производ€тс€
					{
...
//------------------------------------------------------------------------------
	...
	{
		status = PrimaryData::LessMinimumEnergy;//отбраковка минимальна€ энерги€
		return;
	}
...
//	/* не удал€ть
	...
	{
		status = PrimaryData::GreaterMaximumEnergy;//отбраковка максимальна€ энерги€
		return;
	}
	//*/
//-----------------------------------поиск пика	
